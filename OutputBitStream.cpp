 ...
 *
 * <P>This class wraps any {@link std::ostream} so that you can treat it as
 * <em>bit</em> stream. Constructors and methods closely resemble those of
 * {@link std::ostream}. Data can be added to such a stream in several ways:
 ...
 * heavy they can be disabled using a {@linkplain OutputBitStream#OutputBitStream(std::ostream, boolean) suitable constructor}.
 *
 * </ul>
 *
...
class OutputBitStream
{
	...
	int *GAMMA = new int[MAX_PRECOMPUTED], *DELTA = new int[MAX_PRECOMPUTED], *ZETA_3 = new int[MAX_PRECOMPUTED],
		*SHIFTED_GAMMA = new int[MAX_PRECOMPUTED];

	/* We load all precomputed arrays from resource files,
	 * to work around the limit on   initialiser code. */
	...
	// RepositionableStream repositionableStream;
...
public:
	...
	OutputBitStream(vector<uint8_t> a)
	{
		os = NULL;
		free = 8;
		buffer = a;
	 ...
	 */
	// OutputBitStream(String name, int bufSize) throws FileNotFoundException
	// {
	 ...
	 * @param name the name of the file.
	 */
	// OutputBitStream(String name) throws FileNotFoundException
	// {
	 ...
	 * @param bufSize the size in byte of the buffer; it may be 0, denoting no buffering.
	 */
	// OutputBitStream(File file, int bufSize) throws FileNotFoundException
	// {
	 ...
	 * @param file the file.
	 */
	// OutputBitStream(File file) throws FileNotFoundException
	// {
	 ...
	 * byte buffer and delegate to the {@link std::ostream#flush()} method of
	...
	int writeInCurrent(int b, int len) throws IOException
	{
	 ...
	 */
	long write(byte[] bits, long len) throws IOException
	{
	...
	long write(byte[] bits, long offset, long len) throws IOException
	{
	...
	long writeByteOffset(byte[] bits, int offset, long len) throws IOException
	{
	...
	int writeBit(int bit) throws IOException
	{
		if (bit < 0 || bit > 1)
	...
	int write(BooleanIterator i) throws IOException
	{
		int count = 0;
		boolean bit;
		while (i.hasNext())
	...
	long writeLongUnary(long x) throws IOException
	{
		if (x < 0)
			throw new IllegalArgumentException("The argument " + x + " is negative");

		if (x < free)
			return writeInCurrent(1, (int)x + 1);
	...
	int writeGamma(int x) throws IOException
	{
		...
		if (x < MAX_PRECOMPUTED)
			return writeInt(GAMMA[x], GAMMA[x] >>> 26);
	...
	long writeGammas(int[] a, int count) throws IOException
	{
		long l = 0;
	...
	int writeLongGamma(long x) throws IOException
	{
		if (x < 0)
			throw new IllegalArgumentException("The argument " + x + " is negative");
		if (x < MAX_PRECOMPUTED)
			return writeInt(GAMMA[(int)x], GAMMA[(int)x] >>> 26);
	...
	int writeShiftedGamma(int x) throws IOException
	{
		if (x < 0)
			throw new IllegalArgumentException("The argument " + x + " is negative");
		if (x < MAX_PRECOMPUTED)
			return writeInt(SHIFTED_GAMMA[x], SHIFTED_GAMMA[x] >>> 26);
	...
	int writeLongShiftedGamma(long x) throws IOException
	{
		if (x < 0)
			throw new IllegalArgumentException("The argument " + x + " is negative");
		if (x < MAX_PRECOMPUTED)
			return writeInt(SHIFTED_GAMMA[(int)x], SHIFTED_GAMMA[(int)x] >>> 26);
	...
	long writeShiftedGammas(int[] a, int count) throws IOException
	{
		long l = 0;
		for (int i = 0; i < count; i++)
		{
	...
	int writeDelta(int x) throws IOException
	{
		if (x < 0)
			throw new IllegalArgumentException("The argument " + x + " is negative");
		if (x < MAX_PRECOMPUTED)
			return writeInt(DELTA[x], DELTA[x] >>> 26);
	...
	int writeLongDelta(long x) throws IOException
	{
		if (x < 0)
			throw new IllegalArgumentException("The argument " + x + " is negative");
		if (x < MAX_PRECOMPUTED)
			return writeInt(DELTA[(int)x], DELTA[(int)x] >>> 26);
	...
	long writeDeltas(int[] a, int count) throws IOException
	{
		long l = 0;
		for (int i = 0; i < count; i++)
		{
	...
	int writeMinimalBinary(int x, int b) throws IOException
	{
		if (b < 1)
	...
	int writeMinimalBinary(int x, int b, int log2b) throws IOException
	{
		if (x < 0)
			throw new IllegalArgumentException("The argument " + x + " is negative");
		if (b < 1)
	...
	int writeLongMinimalBinary(long x, long b) throws IOException
	{
		if (b < 1)
			throw new IllegalArgumentException("The bound " + b + " is not positive");
	...
	int writeLongMinimalBinary(long x, long b, int log2b) throws IOException
	{
		if (x < 0)
			throw new IllegalArgumentException("The argument " + x + " is negative");
		if (b < 1)
		...
		if (x < m)
			return writeLong(x, log2b);
		else
			return writeLong(m + x, log2b + 1);
	}
	...
	int writeGolomb(int x, int b) throws IOException
	{
	...
	int writeGolomb(int x, int b, int log2b) throws IOException
	{
		if (x < 0)
			throw new IllegalArgumentException("The argument " + x + " is negative");
		if (b < 0)
	...
	long writeLongGolomb(long x, long b) throws IOException
	{
	...
	long writeLongGolomb(long x, long b, int log2b) throws IOException
	{
	...
	int writeSkewedGolomb(int x, int b) throws IOException
	{
		if (x < 0)
			throw new IllegalArgumentException("The argument " + x + " is negative");
		if (b < 0)
	...
	long writeLongSkewedGolomb(long x, long b) throws IOException
	{
		if (x < 0)
			throw new IllegalArgumentException("The argument " + x + " is negative");
		if (b < 0)
		...
		long i = Fast.mostSignificantBit(x / b + 1);
		long l = writeLongUnary(i);
		long M = ((1L << i + 1) - 1) * b;
		long m = (M / (2 * b)) * b;
	...
	int writeZeta(int x, int k) throws IOException
	{
		if (x < 0)
			throw new IllegalArgumentException("The argument " + x + " is negative");
		if (k < 1)
			throw new IllegalArgumentException("The shrinking factor " + k + " is not positive");
		if (k == 3 && x < MAX_PRECOMPUTED)
			return writeInt(ZETA_3[x], ZETA_3[x] >>> 26);
	...
	int writeLongZeta(long x, int k) throws IOException
	{
		if (x < 0)
			throw new IllegalArgumentException("The argument " + x + " is negative");
		if (k < 1)
			throw new IllegalArgumentException("The shrinking factor " + k + " is not positive");
		if (k == 3 && x < MAX_PRECOMPUTED)
			return writeInt(ZETA_3[(int)x], ZETA_3[(int)x] >>> 26);
	...
	int writeNibble(int x) throws IOException
	{
		if (x < 0)
			throw new IllegalArgumentException("The argument " + x + " is negative");
	...
	int writeLongNibble(long x) throws IOException
	{
		if (x < 0)
			throw new IllegalArgumentException("The argument " + x + " is negative");
	 ...
	 */
	void copyFrom(InputBitStream ibs, long length) throws IOException
	{
		byte[] buffer = new byte[64 * 1024];
		while (length > 0)
		{
			int toRead = (int)Math.min(length, buffer.length * Byte.SIZE);
